...
### v1.9.0 (2026-06-26)
**UI 风格重构 — 从深色极光转为浅色柔和玻璃风格：**

**全局色调重构：**
- 主色调：深蓝→浅蓝 `#93b9eb` / `#7ba3d6` / `#8cc8e1`（降饱和、提亮度）
- 背景：多层径向渐变流光系统（3 层伪元素 + 4 个小光斑，共 7 个独立动画）
- 基底色：`#f4f7fc` 柔和浅灰蓝

**侧边栏重构：**
- 从深色 `rgba(15,23,42,0.92)` 改为浅色玻璃 `rgba(255,255,255,0.25)` + `blur(24px)`
- 使用 `@supports` 优雅降级：不支持 backdrop-filter 时使用 `rgba(255,255,255,0.95)`
- 极光光晕：5 色降饱和版本（柔和蓝/青/绿/紫/粉）
- 右侧边缘：改为 1px 淡色分割线（去掉发光动画）
- 菜单项文字：白色→深灰色 `#475569`，激活态柔和蓝 `#6a9ad4`
- 菜单项 hover：图标上浮 `translateY(-2px)` + 颜色高亮
- 活跃菜单左侧指示条：4px 宽，位置微调 `left: -4px`

**Header 重构：**
- 环绕流光边框：降饱和柔和色 + 动画从 3s 放慢到 4s
- Logo 渐变色：`#3b82f6/#06b6d4` → `#93b9eb/#8cc8e1`
- 用户头像渐变：同步柔和化
- Hover 效果：颜色统一为 `#7ba3d6`

**路由优化：**
- 添加 `keep-alive` 缓存页面组件，切换菜单不再重新加载
